    ...
    def forward_seg(self, data_dict, pr_dict):
        ...
        for b in range(feat[0].shape[0]):
            for gt_bbox, gt_roi in zip(gt_bboxes[b], gt_rois[b]):
                ...
                # add ----------------------------------------------------------------------------------------------
                grids = self.rend_grids_sampling_train(pr_roi, (pr_roi.shape[0]*pr_roi.shape[1])//8) # 1 x N x 2
                if grids.shape[1] == 0:
                    continue
                # pr_1
                sampled_pts0 = self.point_sample(feat_rois[0], grids, mode='bilinear').squeeze(-1)  # 1 x 64 x N
                pr_pts_class0 = self.point_head0(sampled_pts0)
                pr_dict['pr_pts_class0'].append(pr_pts_class0.squeeze(0).squeeze(0))
                # gt
                gt_pts_class = self.point_sample(gt_roi, grids, mode='bilinear').squeeze(-1)
                pr_dict['gt_pts_class0'].append(gt_pts_class.squeeze(0).squeeze(0))
                # pr_2
                if len(feat_rois)>1:
                    sampled_pts1 = self.point_sample(feat_rois[1], grids, mode='bilinear').squeeze(-1)
                    pr_pts_class1 = self.point_head1(torch.cat([sampled_pts0,sampled_pts1], dim=1))
                    pr_dict['pr_pts_class1'].append(pr_pts_class1.squeeze(0).squeeze(0))
                    pr_dict['gt_pts_class1'].append(gt_pts_class.squeeze(0).squeeze(0))
                # --------------------------------------------------------------------------------------------------
        return pr_dict
